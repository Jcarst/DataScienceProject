...
The data set we were given is exceptionally messy. There are multiple fields that are not useful (date of birth being one) and others that are inusable in their current state (jail in, jail out). The first thing we did was turn jail in and jail out from dates into one column that calculated total days spent in jail. Then we get rid of name and dob. After that we got rid of other variables based upon our needs for the specific models we created.
...
#### Equation
$$
\textrm{recidivation odds} = e^{0.274}\cdot e^{0.473_{sex=Male}}\cdot e^{-0.037_{age}}\cdot e^{-.0028_{daysInJail}}\cdot e^{0.371ln(priors)}
$$
...
Our equation will give us the odds of a criminal recidivating. The variable constants may seem small but they are exponents, any small change to them will change the odds by no small factor.
...
$$
\textrm{Error Rate} = 0.2926463
$$

#### Equation
$$
\textrm {Recidivation odds} = e^{-1.043}\cdot e^{0.508_{sex=male}}\cdot e^{-0.02_{age}}\cdot e^{0.164_{RiskRecidDecileScore}}\cdot e^{-0.004_{daysInJail}}\cdot e^{0.261_{ln(priors)}}
$$
...
Above is a summary of the logistic model created to predict whether or not a convict will recidivate. If we check the full summery tab we can see that all of our variables are statistically significant. We also have the error tab and the equation tab.

Our model is better with the RiskRecidDecileScore, meaning that the test the criminals were given to predict recidivation rates was helpful in constructing a predictive model.

### Summerizing our logistic regression models

The null model for our recidivism prediction would be to take our data and find whatever category was most common for recidivation (in this case it is most common to not recidivate) and simply predict that every criminal won't recidivate. This would give us an error of $0.3482$ meaning that 34% of criminals that we predicted would not recidivate did revidivate. Both of our models had an error rate of about 29%, meaning that our models were about 5% better than just guessing. This isn't ideal, but it is better. However, we find it unsatisfactory when considering that a model like this, if put into use, could effect the outcome of a criminal's ability to receive parole. We wouldn't be comfortable using any model with something so important and sensetive unless we had a lower error rate.
...
#### Equation
$$
  \textrm {Risk Recid Decile Score} = 7.371 - 0.099_{age} - 1.165_{race=hispanic} - 1.317_{race=other} - 0.83_{race=white} + 0.337_{juvFelonyCount} + 0.279_{juvOtherCount} + 0.272_{priors}
$$
...
Above is our model for predicting the Risk Recidivism Score for a criminal. It is interesting to not that while race wasn't significant enough to include it as a factor for our previous logistic models, it is a significant factor here. For example, if the criminal's race is hispanic, the model will take .77 off of their score making them less of a recidivation risk. This means that it could be that whatever test the criminals took adjusted their score based upon race somehow. That seems unlikely, as most tests wouldn't outright ask for a criminal's race, but perhaps the answers to some of it's questions are good predictors of race.
...
#### Equation
$$
\textrm {Risk Violence Decile Score} = 7.223 + 0.471_{sex=male} - 0.119_{age} - 0.772_{race=hispanic}-0.855_{race=other}-0.650_{race=white}+0.370_{juvFelonyCount}+0.139_{juvMisdemeanerCount}+0.315_{juvOtherCount}+0.123_{priors}
$$
...
Our RMSE is better for predicting violence than it is for predicting recidivism. Our recidivation model doesn't account for gender which is probably a good indication of violence in our violence model. This probably means that whatever test the criminals take accounts for male's being more prone to violence. If a criminal is a male in our model, it adds .47 points to their violence score. Interestingly, the score starts out pretty high (7.222 according to the intercept) and is subtracted from only on account of age and race. A criminal will have a low violence score if they are an elderly woman of a race other than hispanic, black, or white.
...
$$
\textrm {Odds of Recidivation} = e^{-1.586}\cdot e^{-0.013_{age}}\cdot e^{0.150_{priors}}\cdot e^{0.141_{RiskRecidDecileScore}}
$$
...
Above we have our model for predicting recidivism in females. The model isn't very good, considering how it is only about .01 or 1% better than our null hypothesis. 
...
$$
\textrm {Odds of Recidivation} = e^{-0.038}\cdot e^{-0.039_{age}}\cdot e^{0.096_{priors}}\cdot e^{0.139_{RiskRecidDecileScore}}\cdot e^{-0.003_{daysInJail}}
$$
...
We can see that for males the days spent in jail is a significant factor where for females it was not. Our prediction for males is much better as well, with a null hypothesis of 0.39 and our model's error being 0.31. This could be for a variety of reasons, perhaps it is simply more difficult to model the behavior of female criminals. We can see that for men age is a heavier variable, being -0.039 for men and -0.013 for women. The variables for females might be heavier just because there are less to try and predict female recidivation. 
